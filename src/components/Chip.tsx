...
export const Chip = ({ color = 'default', children, className = '' }: ChipProps) => {
  return (
    <div
      className={`inline-flex items-center px-5 py-1 text-sm font-medium rounded-full transition-colors duration-200 cursor-pointer ${colorClasses[color]} ${className}`}
    >
      {children}
    </div>
  );
